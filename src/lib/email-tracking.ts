...
// DB row typings for email_logs (partial, only fields we use)
export interface EmailLogRow {
  id: string;
  event_id?: string | null;
  guest_id?: string | null;
  email_type: EmailLogData['emailType'];
...
const extractApiResult = (d: unknown): { success: boolean; error?: string; id?: string } => {
  if (!d || typeof d !== 'object') return { success: false, error: 'invalid response' };
  const obj = d as Record<string, unknown>;
  const nested = obj['data'];
  if (nested && typeof nested === 'object') {
    const nestedObj = nested as Record<string, unknown>;
    if (typeof nestedObj['id'] === 'string') return { success: Boolean(obj['success'] || nestedObj['success']), id: nestedObj['id'], error: typeof obj['error'] === 'string' ? (obj['error'] as string) : undefined };
  }
  return { success: Boolean(obj['success']), id: typeof obj['id'] === 'string' ? (obj['id'] as string) : undefined, error: typeof obj['error'] === 'string' ? (obj['error'] as string) : undefined };
};
...
export const sendEmailWithTracking = async ({ to, subject, html }: EmailTemplate, logData: EmailLogData): Promise<SendResult> => {
  try {
    ...
    console.log('sendEmailWithTracking - payload:', { to: to, subject, html, from: fromOverride, metadata: {
      ...
      templateBody: logData.templateBody || null
    }});
...
export const resendEmail = async (emailLogId: string) => {
  ...
  const emailTypeCandidate = String(emailLog.email_type || '');
  const allowedTypes = ['invitation', 'confirmation', 'reminder', 'test'] as const;
...
export const emailTemplates = {
  ...
  <p>We received a request to reset your password for PartyHause.</p>
    ...
    html: `
      ...
      <body>
        <div class="container">
          ...
          <div class="footer">
            <p>Powered by <a href="#" class="brand">PartyHause</a></p>
