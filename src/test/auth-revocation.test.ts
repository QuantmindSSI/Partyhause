...
describe('revocable auth routes', () => {
  ...
  it('increments the token epoch on reset and issues a token from the NEW epoch', async () => {
    ...
    // The route signs from the row `update` RETURNS, so the mock must carry the
    // post-increment epoch. Signing from the pre-update read would mint a token
    // one epoch behind, which requireAuth rejects.
    ...
    // A reset revokes every OTHER session and keeps this one. Issuing nothing
    // would revoke an attacker's session and also sign out the person who just
    // proved control of the mailbox, at the exact moment they proved it.
    expect(typeof body.token).toBe('string');
    expect(body.user).toMatchObject({ id: 'user-2', email: 'reset@example.com' });

    // The point of the test: the epoch inside the token is the incremented one.
    // A token minted from the pre-update read would carry 3 and be rejected by
    // requireAuth on the very next request, so the reset would look like it
    // worked and then 401 forever.
    const claims = JSON.parse(
      Buffer.from((body.token as string).split('.')[1], 'base64url').toString('utf8'),
    ) as { token_version: number };
    expect(claims.token_version).toBe(4);
  });
