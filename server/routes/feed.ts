...
router.get('/crew', requireAuth, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    let cursorCondition: { published_at: { lt: Date } } | undefined = undefined;
    if (cursor) {
      ...
      if (cursorPost) {
        cursorCondition = { published_at: { lt: cursorPost.published_at } };
      }
    }
    ...
    const posts = await prisma.partycrewPost.findMany({
      ...
      take: limit * 3, // Fetch 3x to have ranking candidates
    });
    ...
    // Mark posts as viewed (upsert with unique constraint on user_id + post_id)
    ...
    const nextCursor =
      formattedPosts.length > 0 ? formattedPosts[formattedPosts.length - 1].id : null;
