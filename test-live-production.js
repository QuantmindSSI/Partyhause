...
async function testLiveProduction() {
  ...
  try {
    console.log('\n📱 Testing main site...');
      const response = await fetch('https://partyhause.vercel.app');
    if (response.ok) {
  ...
  try {
    console.log('\n🔗 Testing email webhook...');
      const response = await fetch('https://partyhause.vercel.app/api/email-webhook', {
      method: 'OPTIONS'
    });
